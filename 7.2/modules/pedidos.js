...
export const PedidosManager = {
  ...
  async cargarPedidos() {
    ...
    try {
      // 1️⃣ Cargar nombres de clientes para caché
      ...
      facturasSnap.forEach(docSnap => {
        ...
        // ✅ NUEVO: IGNORAR FACTURAS DE FACTURACIÓN 2.0 (Admin)
        // Estas ya están "aprobadas" por ser creadas por admin, no deben salir en pedidos
        if (data.tipoFactura === 'rapida') return;

        // ✅ CASO A: Estructura Nueva (Documento único es el pedido)
        // Usado por FacturacionRapidaV2 (si no fuera rapida) y CartManager
        if ((data.estado === 'pendiente' || data.estado === 'en_proceso') && !Array.isArray(data.compras)) {
          pedidos.push({
            idFactura: id,
            index: 0, // No aplica índice en estructura plana
            nombreCliente: data.clienteNombre || 'Cliente Web',
            pedido: { 
              fecha: data.fecha, 
              total: data.total, 
              ...
              estado: data.estado,
              tipoFactura: data.tipoFactura || 'web'
            }
          });
        }

        // ✅ CASO B: Estructura Antigua (Array de compras dentro del documento)
          ...
          data.compras.forEach((c, i) => {
            if (c.estado === 'pendiente' || c.estado === 'en_proceso') {
              pedidos.push({
                idFactura: id,
                index: i,
                nombreCliente: this._clientesCache[id] || data.clienteNombre || 'Cliente',
                pedido: c
              });
            }
          });
      ...
      container.innerHTML = pedidos.map(p => `
        <div class="pedido-card">
          <div class="pedido-header">
            ...
            ${p.pedido.tipoFactura === 'web' ? '<span class="badge-web"> Web</span>' : ''}
          </div>
  ...
  async aprobarPedido(idFactura, index) {
    ...
    try {
      ...
      // ✅ Determinar si es estructura nueva o antigua
      if (Array.isArray(data.compras)) {
        compra = data.compras[index];
      } else {
        compra = data; // Estructura plana (nueva)
      }
      ...
      if (puntosGanados > 0) {
        try {
          const clienteId = data.clienteId || idFactura;
          ...
          await updateDoc(clienteRef, {
            ...
            historialPuntos: arrayUnion({
              ...
              motivo: 'Pedido #' + (index + 1),
              autor: 'sistema'
            })
          });
          UI.toast(`🏆 ¡Cliente ganó ${puntosGanados} puntos!`, 'success');
        } catch (err) {
      ...
      if (Array.isArray(data.compras)) {
        const comprasActualizadas = [...data.compras];
        ...
        // Estructura plana: cambiar estado a despachado
        await updateDoc(doc(DB.db, "facturas_rapidas", idFactura), { estado: 'despachado' });
      }
  ...
  async verificarNotificacionPedidos() {
    ...
    if (sessionStorage.getItem('banner_pedidos_cerrado')) return;
    try {
      const facturasSnap = await getDocs(collection(DB.db, "facturas_rapidas"));
      ...
      for (const docSnap of facturasSnap.docs) {
        ...
        // Ignorar facturas rápidas (admin)
        if (data.tipoFactura === 'rapida') continue;

        if ((data.estado === 'pendiente' || data.estado === 'en_proceso') && !Array.isArray(data.compras)) {
          hayPendientes = true; break;
        }
        if (Array.isArray(data.compras) && data.compras.some(c => c.estado === 'pendiente' || c.estado === 'en_proceso')) {
          hayPendientes = true; break;
        }
      }
  ...
  async cancelarPedido(idFactura, index) {
    ...
    try {
      ...
      if (Array.isArray(data.compras)) {
        const comprasActualizadas = [...data.compras];
        comprasActualizadas[index] = { ...comprasActualizadas[index], estado: 'anulado' };
        await updateDoc(doc(DB.db, "facturas_rapidas", idFactura), { compras: comprasActualizadas });
      } else {
        await updateDoc(doc(DB.db, "facturas_rapidas", idFactura), { estado: 'anulado' });
      }
