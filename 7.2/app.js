...
import InventoryManager from './modules/inventory-manager.js'; // ✅ NUEVO: Importar módulo
import VelaCustomizer from './modules/vela-customizer.js';
import VelasAdminManager from './modules/velas-admin.js';
...
window.InventoryManager = InventoryManager; // ✅ NUEVO: Exponer módulo
...
window.VelaCustomizer = VelaCustomizer;
window.VelasAdminManager = VelasAdminManager;
...
// ✅ INICIALIZAR PERSONALIZADOR DE VELAS
  if (window.VelaCustomizer) await window.VelaCustomizer.init();
...
if (btnAdmin && adminMenu) {
  ...
  adminMenu.addEventListener('click', (e) => {
    ...
    const routeMap = {
      ...
      inventory: { manager: 'InventoryManager', method: 'mostrarPanel' }, // ✅ ACTUALIZADO
      inventario: { manager: 'InventoryManager', method: 'mostrarPanel' }, // ✅ NUEVO (soporte español)
      ...
      impresion: { manager: 'ImpresionManager', method: 'mostrarPanel' },
      diagnostico: { manager: 'DiagnosticoFacturas', method: 'mostrarPanel' },
      velasCustom: { manager: 'VelasAdminManager', method: 'mostrarPanel' } // ✅ NUEVO
    };
...
if (items.length === 0) {
  ...
  // 🔍 BUSCA ESTE BLOQUE EN renderCartModal() y REEMPLÁZALO:
lista.innerHTML = items.map(item => `
   <li class="item-carrito">
     <div class="item-info">
       <strong>${item.nombre}</strong>
       <span>${item.variante} ${item.aroma ? `• <b>${item.aroma}</b>` : ''} × ${item.cantidad}</span>
     </div>
