...
export const FacturacionRapidaV2 = {
  ...
  // ✅ NUEVO: Flags para prevenir bloqueos
  _isOpening: false,
  _eventListeners: [],
  _modalAbierto: false,
  ...
  // ✅ CORREGIDO: Protección anti-bloqueo
  async mostrarPanel() {
    if (!Store.get('isAdmin')) { UI.toast('Acceso denegado', 'warning'); return; }

    // ✅ Prevenir múltiples aperturas
    if (this._isOpening || this._modalAbierto) {
      console.log('⚠️ Modal ya abierto o en proceso de apertura');
      return;
    }
    this._isOpening = true;

    try {
      // ✅ 1. LIMPIEZA PREVENTIVA COMPLETA
      ...
      modal.innerHTML = `
        <div class="modal-content modal-xl facturacion-modal">
          <button class="modal-close" onclick="FacturacionRapidaV2.cerrarModal()">✕</button>
      ...
      console.error('Error abriendo facturación:', error);
      UI.toast('❌ Error al abrir facturación', 'error');
    } finally {
      this._isOpening = false;
    }
  },

  // ✅ NUEVO: Cerrar modal y limpiar
  cerrarModal() {
    this._clearEventListeners();
    this._modalAbierto = false;
    UI.modal('modalFacturacionRapida', 'close');
  },
  ...
  // ✅ NUEVO: Helper para agregar listener trackeable
  ...
  async mostrarModalClientes() {
    ...
    const existing = document.getElementById('modalSeleccionCliente');
    ...
    const buscarInput = document.getElementById('buscarClienteModal');
    if (buscarInput) {
      const handler = (e) => {
  ...
  cerrarModalClientes() {
    const modal = document.getElementById('modalSeleccionCliente');
  ...
  // ✅ CORREGIDO: attachEvents con limpieza previa
  attachEvents() {
    this._clearEventListeners();

    const frNuevoCliente = document.getElementById('frNuevoCliente');
    if (frNuevoCliente) {
      this._addTrackedListener(frNuevoCliente, 'click', () => this.nuevoCliente());
    }

    const frBuscarProducto = document.getElementById('frBuscarProducto');
    if (frBuscarProducto) {
      this._addTrackedListener(frBuscarProducto, 'input', () => this.renderProductos());
    }

    const btnRecargar = document.getElementById('btnRecargarProductos');
    if (btnRecargar) {
      this._addTrackedListener(btnRecargar, 'click', () => {
        this.prepararProductos();
        this.renderProductos();
    ...
    const frDescuento = document.getElementById('frDescuento');
    if (frDescuento) {
      this._addTrackedListener(frDescuento, 'input', () => this.calcularTotales());
    }

    const frLimpiar = document.getElementById('frLimpiar');
    if (frLimpiar) {
      this._addTrackedListener(frLimpiar, 'click', () => this.limpiarFactura());
    }

    const frGuardar = document.getElementById('frGuardarFactura');
    if (frGuardar) {
      this._addTrackedListener(frGuardar, 'click', () => this.guardarFactura());
    }

    const frNueva = document.getElementById('frNuevaFactura');
    if (frNueva) {
      this._addTrackedListener(frNueva, 'click', () => this.prepararNuevaFactura());
    }
  },
  ...
  calcularTotales() {
    const subtotal = this.carritoFactura.reduce((sum, i) => sum + (i.subtotal || 0), 0);
    const descuento = parseInt(document.getElementById('frDescuento')?.value) || 0;
    const total = Math.max(0, subtotal - descuento);
    const frSubtotal = document.getElementById('frSubtotal');
    const frTotal = document.getElementById('frTotal');
    if (frSubtotal) frSubtotal.textContent = `₡${subtotal.toLocaleString()}`;
    if (frTotal) frTotal.textContent = `₡${total.toLocaleString()}`;
    return { subtotal, descuento, total };
  },

  // ✅ FUNCIÓN CORREGIDA CON PROTECCIÓN ANTI-BLOQUEO
  ...
  prepararNuevaFactura() {
    ...
    const frBuscarCliente = document.getElementById('frBuscarCliente');
    if (frBuscarCliente) frBuscarCliente.value = '';

    const frClienteInfo = document.getElementById('frClienteInfo');
    if (frClienteInfo) {
      frClienteInfo.classList.add('hidden');
      frClienteInfo.innerHTML = '';
    }

    const frDescuento = document.getElementById('frDescuento');
    if (frDescuento) frDescuento.value = 0;

    const frMetodoPago = document.getElementById('frMetodoPago');
    if (frMetodoPago) frMetodoPago.value = 'contado';
